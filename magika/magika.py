...
class Magika:
    def __init__(
        ...
        if model_dir is not None:
            ...
            if not self.model_dir.is_dir():
                raise Exception(
                    f'User-specified model dir "{str(self.model_dir)}" not found'
                )
            ...
            if not self.model_path.is_file() or not self.model_config_path.is_file():
                ...
                model_config_url = (
                    GC_BUCKET_URL_PREFIX + f"model_config-{MODEL_HASH}.json"
                )
                self.l.info(
                    f"Downloading DL model config {model_config_url} => {self.model_config_path}"
                )
        ...
        self.target_labels_space = np.array(
    ...
    def get_content_types(self, paths: List[Path]) -> List:
        ...
        for path in tqdm(paths, disable=not (self.verbose or self.debug)):
            features.append(
                (path, self.extract_features(path, beg_size, mid_size, end_size))
            )
        elapsed_time = time.time() - start_time
        ...
        if len(features) > 0:
            ...
            for (path, _), ct_label, score in zip(
                ...
                if self.mime_output_flag:
                    ...
                    entry["output"]["mime_type"] = self.ctm.get_mime_type(
                        output_ct_label
                    )
    ...
    def get_output_ct_label(
        self, path: Path, ct_label: str, score: float
    ) -> Tuple[str, float]:
        if path.stat().st_size == 0:
            ...
            score >= self.high_score_threshold
            or score >= self.ctm.get_or_raise(ct_label).threshold
        ):
    ...
    def extract_features(
        self, file_path: Path, beg_size: int, mid_size: int, end_size: int
    ) -> Dict[str, List[int]]:
        ...
        if file_size < 2 * block_size:
            ...
            with open(file_path, "rb") as f:
                content = f.read()
            beg, mid, end = self.extract_features_from_content(

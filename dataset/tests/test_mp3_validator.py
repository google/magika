...
def layer2_frame(bitrate_index=8):
    header = bytes([0xFF, 0xFD, bitrate_index << 4, 0x00])  # MPEG-1 layer II, 44.1 kHz
    length = 144 * mp3.BITRATES[(1, 2)][bitrate_index] * 1000 // 44100
    return header + b"\0" * (length - 4)


def test_layer_ii_frames_are_named_mp2():
    result = mp3.validate(id3v2() + layer2_frame() * 5 + layer2_frame(10), frozenset())
    assert (result.status, result.format_id) == ("pass", "mp2")
    assert "6 layer II frames" in result.detail


def test_a_stream_switching_layers_fails():
    data = layer2_frame() * 3 + frame() * 3
    result = mp3.validate(data, frozenset())
    assert (result.status, result.format_id) == ("fail", "mp2")

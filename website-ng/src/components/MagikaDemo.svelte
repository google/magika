	...
	{#if loadingProgress === 100}
		<Tabs.Root value="text" class="w-full ">
			...
			<Tabs.Content value="text" class="w-full flex flex-col">
				<div>
					Type or paste text below to test out Magika. The processing
					happens entirely in your browser - the text won't be
					uploaded anywhere else. The content type is detected as you
					type.
				</div>
				<Label for="message"
				...
				{#if textClassification}
					<div class="mt-2 flex gap-2 text-sm">
						...
						<span class="font-semibold"
							>{textClassification?.topLabel}</span
						>
						<span>using model</span>
						<span class="font-semibold"
							>{textClassification?.modelVersion ??
								"unknown"}</span
						>
					</div>
				{/if}
			...
			<Tabs.Content value="file">
				<div class="flex w-full flex-col gap-2 p-6">
					<div>
						You can drop your files below to test out Magika. The
						processing happens entirely in your browser - the files
						won't be uploaded anywhere else.
					</div>
					<FileDropZone
						{onUpload}
						{onFileRejected}
					...
					<div class="flex flex-col gap-2">
						{#each files as file, i (file.name)}
							<Card
								class="flex px-5 justify-between gap-2 relative"
							>
								<Button
									variant="ghost"
									size="icon"

...
JNLP = b"""<?xml version="1.0" encoding="utf-8"?>
<jnlp spec="1.0+" codebase="https://example.org/app">
  <information><title>App</title><vendor>Example</vendor></information>
  <resources><jar href="app.jar" main="true"/></resources>
  %s
</jnlp>"""


def test_a_jnlp_descriptor_needs_information_and_one_description():
    result = xml_family.validate(JNLP % b'<application-desc main-class="App"/>', frozenset())
    assert (result.status, result.format_id) == ("pass", "jnlp")
    none = xml_family.validate(JNLP % b"", frozenset())
    assert (none.status, none.format_id) == ("fail", "jnlp")
    two = JNLP % b'<application-desc/><applet-desc name="a" main-class="A" width="1" height="1"/>'
    assert xml_family.validate(two, frozenset()).status == "fail"

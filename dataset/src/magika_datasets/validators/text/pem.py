...
def validate(data: bytes, hints: frozenset[str]) -> Observation | None:
    ...
    if b"-----BEGIN " not in head[:4096]:
        return None
    kind = "pem"  # crt names the binary DER class; armor makes a file pem whatever its name

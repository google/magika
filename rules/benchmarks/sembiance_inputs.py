...
def prepare(snapshot, verified, taxonomy, output):
    ...
    for shard in manifest['shards']:
        ...
        for batch in pq.ParquetFile(path).iter_batches(columns=['sha256', 'format_id', 'annotation_json'], batch_size=256, use_threads=False):
            for row in batch.to_pylist():
                sha = row['sha256'].hex()
                a = json.loads(row['annotation_json'])
                accepted = bool(a['evaluation_eligible'])
                annotations.append({'sha256': sha, 'source_format': row['format_id'],
                    'evaluation_eligible': accepted, 'evaluation_basis': a.get('evaluation_basis'),
                    'validation_status': a.get('validation_status'), 'overlap_existing_sha256': a.get('overlap_existing_sha256'),
                ...
                eligible.append(by_hash[sha] | {'truth': row['format_id'], 'ambiguous': False,
    ...
    identity = source['identity'] | {'scope': 'Sembiance evaluation_eligible subset',
        'eligible_samples': len(eligible), 'eligible_classes': len({s['truth'] for s in eligible}),
        ...
        'near_duplicates': 'not certified', 'baseline_dataset': 'unchanged; external evaluation only'}
    c.save_gzip(output / 'inputs.json.gz', {'identity': identity, 'classes': classes,
        'supported': base.get('supported', []), 'samples': eligible})

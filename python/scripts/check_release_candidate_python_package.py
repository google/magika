...
@click.option("--python-client", is_flag=True, help="Use the Python client instead of CLI for validation.")
def main(expected_version, python_client):
    ...
    errors += validate_release_tag(instance_version, expected_version)
...
def validate_release_tag(extracted_version, release_tag):
    ...
    if extracted_version and extracted_version != normalized_release_tag:
        errors.append(f"ERROR: Extracted version {extracted_version} does not match release tag {release_tag}.")
    return errors
...
def check_python_module_version():
    ...
    if "-dev" in module_version:
        errors.append(f"ERROR: Python package version {module_version} contains '-dev'.")
...
def check_rust_cli_version():
    ...
    """
    try:
        result = subprocess.run(["magika", "--version"], capture_output=True, text=True, check=True)
        ...
        if "-dev" in cli_version:
            click.echo(f"ERROR: Rust CLI version {cli_version} contains '-dev'.", err=True)
            sys.exit(1)
        return cli_version

...
class ContentTypesManager():
    ...
    def __init__(self, content_type_config_path: Path = CONTENT_TYPES_CONFIG_PATH, add_automatic_tags: bool = True):
        ...
        self.load_content_types_info(content_type_config_path=content_type_config_path, add_automatic_tags=add_automatic_tags)
    ...
    def get_mime_type(self, content_type_name: str, default: str = ContentType.UNKNOWN_MIME_TYPE) -> str:
        ct = self.get(content_type_name)
        if ct is None: return default
        if ct.mime_type is None: return default
        return ct.mime_type

    def get_group(self, content_type_name: str, default: str = ContentType.UNKNOWN_CONTENT_TYPE_GROUP) -> str:
        ct = self.get(content_type_name)
        if ct is None: return default
        if ct.group is None: return default
        return ct.group

    def get_magic(self, content_type_name: str, default: str = ContentType.UNKNOWN_MAGIC) -> str:
        ct = self.get(content_type_name)
        if ct is None: return default
        if ct.magic is None: return default
        return ct.magic

...
mod tests {
    ...
    /// Opt-in release measurement; JSON is emitted without a timing assertion.
    /// Run with --release --ignored --nocapture --test-threads=1 on an idle host.
    #[test]
    #[ignore = "performance experiment; requires an idle host and release build"]
    fn profile_small_batch_preparation_and_fusion() -> TractResult<()> {
        use std::time::Instant;
        // Keep the baseline explicit after the shipped graph has adopted fusion.
        let baseline_path = std::env::var("MAGIKA_PROFILE_BASELINE_GRAPH").context(
            "set MAGIKA_PROFILE_BASELINE_GRAPH to the unfused revision's model.graph.json",
        )?;
        ...
        for batch in [1, 4] {
            for repeat in 0..5 {
                ...
                for fused in [false, true] {
                    ...
                    let mut bundle =
                        Bundle::read(&baseline, include_bytes!("../models/model.weights"))?;
                    let parse_ns = start.elapsed().as_nanos();
                    let start = Instant::now();
                    let mut model = bundle.model(batch, true)?;
                    let model_ns = start.elapsed().as_nanos();
                    if fused {
                        ensure!(
                            crate::direct_conv::fuse_magika_conv_max(&mut model, batch)?
                                == crate::EXPECTED_CONVOLUTIONS
                        );
                    }
                    let start = Instant::now();
                    let runnable = DefaultRuntime.prepare_with_options(model, &options)?;
                    let plan_ns = start.elapsed().as_nanos();
                    let mut state = runnable.spawn()?;
                    let input: Vec<_> = (0..batch * FEATURE_SIZE)
                        .map(|i| ((i * (repeat + 1) + i / 97) % 257) as i32)
                        .collect();
                    let scores = crate::run_plan(state.as_mut(), &input, batch)?;
                    let (bit_differences, max_error) = if fused {
                        check_cpu_artifact_scores(&reference, &scores)?;
                        (
                            reference
                                .iter()
                                .zip(&scores)
                    ...
                    for _ in 0..10 {
                        crate::run_plan(state.as_mut(), &input, batch)?;
                    }
                    let start = Instant::now();
                    for _ in 0..150 {
                        crate::run_plan(state.as_mut(), &input, batch)?;
                    }
                    let inference_ns = start.elapsed().as_nanos();
                    measurements.push(serde_json::json!({"batch": batch, "repeat": repeat,
                        "fused": fused, "parse_validate_ns": parse_ns, "model_ns": model_ns,
                        "plan_ns": plan_ns, "iterations": 150, "inference_ns": inference_ns,
                        "score_bit_differences": bit_differences, "score_max_absolute_error": max_error,
                        "winning_labels_and_thresholds_equal": true}));
                }
            }
        }
        println!("{}", serde_json::json!({"small_batch_profile": 1, "measurements": measurements}));
        Ok(())
    }

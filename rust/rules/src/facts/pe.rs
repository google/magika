...
//! 3. **Optional header** at `opt = e_lfanew + 24`: `magic` (u16 at 0), `entry_point`
//!    (u32 at 16), `subsystem` (68) and `dll_characteristics` (70). A `magic` other than 0x10b (PE32) or 0x20b
...
pub(super) struct PeFacts {
    ...
    pub subsystem: u16,
    /// Zero for a resource-only image, such as a MUI file.
    pub entry_point: u32,
...
pub(super) fn analyze(first: &[u8], size: u64) -> PeFacts {
    ...
    facts.entry_point = u32_at(first, opt + 16).unwrap_or(0);
...
pub(super) mod tests {
    ...
    impl Image {
        ...
        pub(crate) fn build(&self) -> Vec<u8> {
            ...
            put_u32(&mut optional, 16, self.entry_point);
    ...
    #[test]
    fn a_resource_only_image_has_no_entry_point() {
        assert_eq!(facts_of(&Image { entry_point: 0, ..pe32() }.build()).entry_point, 0);
        assert_eq!(facts_of(&pe32plus().build()).entry_point, 0x1000);
    }

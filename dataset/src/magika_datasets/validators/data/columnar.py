...
FAMILY = "data"
FORMAT_IDS = ("parquet", "orc", "arrow", "geoparquet")
SCOPE = "Parquet: PAR1 magics, footer length, metadata parsed by pyarrow, every column chunk inside the file; GeoParquet when the geo key metadata is JSON naming a primary column present in the schema with per-column encodings. ORC: postscript and footer parsed by pyarrow, stripes inside the file. Arrow IPC: ARROW1 magics, footer parsed, every record batch read. Ten-second subprocess; values not interpreted"
...
def inspect(data: bytes, kind: str) -> tuple[str, str]:
    ...
    try:
        if kind == "parquet":
            ...
            if b"geo" in extra:
                try:
                    document = json.loads(extra[b"geo"])
                    columns, primary = document["columns"], document["primary_column"]
                    names = set(metadata.schema.names)
                    if (
                        isinstance(columns, dict)
                        and primary in columns
                        and set(columns) <= names
                        and all(isinstance(c, dict) and c.get("encoding") for c in columns.values())
                    ):
                        geo = f"; geo primary column {primary}"
                except (ValueError, KeyError, TypeError):
                    pass  # a malformed geo key leaves valid Parquet, just not proven GeoParquet
            return (
                "pass",
                f"{metadata.num_row_groups} row groups and {metadata.num_columns} columns bounded{geo}; codecs {','.join(sorted(codecs)) or 'none'}",
            )
        if kind == "orc":
...
def validate(data: bytes, hints: frozenset[str]) -> Observation | None:
    ...
    if kind == "parquet" and "; geo primary column" in detail:
        kind = "geoparquet"
    tags = ()

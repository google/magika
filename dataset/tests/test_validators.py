...
def test_validation_groups_preserve_disagreement_and_require_manual_evidence():
    ...
    # A failing pdf check refutes the detector's claim; it does not contradict the png proof.
    assert classify({"vt_markings": {"magika": "pdf"}}, failed, formats) == "validated_auto"
    self_refuted = {
        "observations": [
            dict(report["observations"][0]),
    ...
    assert (
        classify({}, self_refuted, formats) == "need_review"
    )  # a proof its own check fails is none
    assert classify({"label_status": "accepted"}, {"observations": []}, formats) == "need_review"
    manual = {
        "manual_validation": {
            "reviewer": "reviewer",
            "decision": "validated",
...
def test_automatic_relabel_settles_label_and_keeps_hard_case():
    ...
    assert result["label_status"] == "validated_auto" and result["hard_case"]
...
def test_llm_label_requires_attributed_evidence():
    ...
    assert apply_label(annotation, report, {"png": {}})["validation_status"] == "need_review"

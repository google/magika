...
def with_mui_resource(file_type):
    """A PE32 image whose first section holds a resource tree with one MUI resource."""
    data = bytearray(image())
    base, rva = 0x200, 0x1000
    tree = struct.pack("<IIHHHH", 0, 0, 0, 0, 1, 0) + struct.pack(
        "<II", 0x80000000 | 88, 0x80000000 | 24
    )
    tree += struct.pack("<IIHHHH", 0, 0, 0, 0, 0, 1) + struct.pack("<II", 1, 0x80000000 | 48)
    tree += struct.pack("<IIHHHH", 0, 0, 0, 0, 0, 1) + struct.pack("<II", 0x409, 72)
    tree += struct.pack("<IIII", rva + 96, 24, 0, 0)
    tree += struct.pack("<H", 3) + "MUI".encode("utf-16-le")
    tree += struct.pack("<5I", pe.MUI_SIGNATURE, 24, 0x10000, 0, file_type) + b"\0" * 4
    data[base : base + len(tree)] = tree
    struct.pack_into("<II", data, 0x98 + 96 + 16, rva, len(tree))
    return bytes(data)


def test_a_language_resource_file_is_named_mui():
    result = pe.validate(with_mui_resource(0x12), frozenset())
    assert (result.status, result.format_id) == ("pass", "mui")
    assert "MUI resource file type 0x12" in result.detail


def test_a_language_neutral_binary_stays_pe():
    assert pe.validate(with_mui_resource(0x11), frozenset()).format_id == "pe"
    assert pe.validate(image(), frozenset()).format_id == "pe"

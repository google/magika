...
def validate(data: bytes, hints: frozenset[str]) -> Observation | None:
    ...
    if not data.startswith(b"PK\x03\x04"):
        # A file named as a ZIP with no end-of-central-directory record anywhere a reader
        # would look is not one; saying so lets its real format be proven instead.
        if "zip" in hints and b"PK\x05\x06" not in data[-(22 + 0xFFFF) :]:
            return Observation("fail", "No ZIP local header or end of central directory", "zip")
        return None

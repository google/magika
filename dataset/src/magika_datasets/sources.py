...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(description=__doc__, prog="magika-datasets sources")
    parser.add_argument("--samples", type=Path, default=Path("samples.parquet"))
    parser.add_argument("--pool", type=Path, default=Path("repositories.parquet"))
    parser.add_argument("--output", type=Path, default=Path("github-used.parquet"))
    parser.add_argument("--receipt", type=Path, default=Path("github-used-receipt.json"))
    args = parser.parse_args(argv)
    from .acquisition import sha256_file
    from .runtime import private_output, write

    pool = args.pool if args.pool.exists() else None
    receipt = {
        "repositories": export_sources(args.samples, private_output(args.output), pool),
        "samples_sha256": sha256_file(args.samples),
        "inventory_sha256": sha256_file(pool) if pool else None,

...
def test_a_zip_hinted_file_without_a_central_directory_fails():
    import gzip

    data = gzip.compress(b"not a zip at all" * 20)
    result = zipfamily.validate(data, frozenset({"zip"}))
    assert (result.status, result.format_id) == ("fail", "zip")
    assert (
        zipfamily.validate(data, frozenset()) is None
    )  # without the hint the bytes say nothing about ZIP


def test_a_self_extracting_zip_is_not_refuted():
    stub = b"MZ" + b"\0" * 64 + b"PK\x05\x06" + b"\0" * 18
    assert zipfamily.validate(stub, frozenset({"zip"})) is None

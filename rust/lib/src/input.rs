...
fn extract_features(
    ...
    let end = if file_len == buffer_size as u64 {
        // The prefix already holds the whole file, tail included.
...
mod tests {
    ...
    impl Input for &mut CountingInput {
        fn length(&self) -> Result<u64> {
    ...
    fn a_complete_prefix_is_not_read_again_for_the_tail() {
        ...
        for len in [1, 7, 8, 9, 1023, 1024, block_size - 1, block_size, block_size + 1] {
            let mut input = CountingInput {
                bytes: (0..len).map(|i| (i % 256) as u8).collect(),
                reads: Vec::new(),
            };
            let expected = extract_features(config, input.bytes.as_slice(), len as u64).unwrap();
            let actual = extract_features(config, &mut input, len as u64).unwrap();
            assert_eq!(actual, expected, "input length {len}");
            let reads = if len <= block_size {
                vec![(0, len)]
            } else {
                vec![(0, block_size), ((len - block_size) as u64, block_size)]
            };
            assert_eq!(input.reads, reads, "input length {len}");
        }
    }

...
export class ModelFeatures {
  ...
  constructor(public config: Config) {
    this.start = new Uint16Array(this.config.begBytes).fill(
      this.config.paddingToken,
    );
    this.middle = new Uint16Array(this.config.midBytes).fill(
      this.config.paddingToken,
    );
    this.end = new Uint16Array(this.config.endBytes).fill(
      this.config.paddingToken,
    );

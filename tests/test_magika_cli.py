...
def test_magika_cli_with_basic_test_files():
    test_files_paths = get_basic_test_files_paths()
...
def test_magika_cli_with_basic_test_files_and_json_output():
    ...
    for n in [1, 2, 5, 10, len(test_files_paths)]:
        ...
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, json_output_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['--json'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, json_output_flag=True)
...
def test_magika_cli_with_basic_test_files_and_jsonl_output():
    ...
    for n in [1, 2, 5, 10, len(test_files_paths)]:
        ...
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, jsonl_output_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['--jsonl'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, jsonl_output_flag=True)
...
def test_magika_cli_with_basic_test_files_and_probability():
    ...
    for n in [1, 2, 5, 10, len(test_files_paths)]:
        stdout, stderr = run_magika_cli(test_files_paths[:n], with_probability_flag=True)
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, with_probability_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['-p'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, with_probability_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['--with-probability'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, with_probability_flag=True)
...
def test_magika_cli_with_basic_test_files_and_mime_output():
    ...
    for n in [1, 2, 5, 10, len(test_files_paths)]:
        ...
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, mime_output_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['-i'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, mime_output_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['--mime'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, mime_output_flag=True)
...
def test_magika_cli_with_basic_test_files_and_compatibility_mode():
    ...
    for n in [1, 2, 5, 10, len(test_files_paths)]:
        stdout, stderr = run_magika_cli(test_files_paths[:n], compatibility_mode_flag=True)
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, compatibility_mode_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['-c'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, compatibility_mode_flag=True)

        stdout, stderr = run_magika_cli(test_files_paths[:n], extra_cli_options=['--compatibility-mode'])
        check_magika_cli_output_matches_expected(test_files_paths[:n], stdout, stderr, compatibility_mode_flag=True)


def check_magika_cli_output_matches_expected(samples_paths: List[Path], stdout: str, stderr: str, **kwargs: Dict[str, Any]):
    ...
    predicted_cts = get_magika_cli_output_from_stdout_stderr(samples_paths, stdout, stderr, **kwargs)
    assert len(predicted_cts) > 0
    assert len(samples_paths) == len(predicted_cts)
    for file_path, output in predicted_cts:
        file_ext = file_path.suffix.lstrip('.')
        if mime_output_flag:
            expected_output = ctm.get_mime_type(ctm.get_ct_by_ext(file_ext).name)
        elif compatibility_mode_flag:
            expected_output = ctm.get_magic(ctm.get_ct_by_ext(file_ext).name)
        else:
            ct_group = ctm.get_group(ctm.get_ct_by_ext(file_ext).name)
            ct_label = ctm.get_ct_by_ext(file_ext).name
...
def test_magika_cli_with_multiple_copies_of_the_same_file():
    ...
    test_file_path = get_one_basic_test_file_path()
...
def test_magika_cli_with_big_file():
    ...
    for sample_size in [1000, 10000, 1_000_000, 1_000_000_000, 10_000_000_000]:
        with tempfile.TemporaryDirectory() as td:
            ...
            write_random_file_with_size(sample_path, sample_size)
...
def test_magika_cli_with_colors():
    ...
    stdout, stderr = run_magika_cli([test_file_path], with_colors_flag=True, mime_output_flag=True)
    assert stdout.find('\033') >= 0 or stderr.find('\033') >= 0
    stdout, stderr = run_magika_cli([test_file_path], with_colors_flag=True, verbose_flag=True, debug_flag=True)
    assert stdout.find('\033') >= 0 or stderr.find('\033') >= 0
...
def test_magika_cli_with_no_colors():
    ...
    stdout, stderr = run_magika_cli([test_file_path], with_colors_flag=False, mime_output_flag=True)
    assert stdout.find('\033') == -1 and stderr.find('\033') == -1
    stdout, stderr = run_magika_cli([test_file_path], with_colors_flag=False, verbose_flag=True, debug_flag=True)
    assert stdout.find('\033') == -1 and stderr.find('\033') == -1
...
def get_magika_cli_output_from_stdout_stderr(samples_paths: List[Path], stdout: str, stderr: str, **kwargs: Dict[str, Any]) -> List:
...
def run_magika_cli(
        ...
        extra_cli_options: Optional[List[str]] = None) -> Tuple[str, str]:
...
def get_lines_from_stream(stream) -> List[str]:
...
def write_random_file_with_size(sample_path: Path, sample_size: int):

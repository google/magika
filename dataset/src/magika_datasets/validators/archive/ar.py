...
"""Unix ar archives, Debian packages, Rust libraries and VMware VIBs: member headers tiling the file."""
...
FAMILY = "archive"
FORMAT_IDS = ("ar", "deb", "rlib", "vib")
SCOPE = "Global magic, member headers (decimal size, `\\x60\\n` terminator), even padding and members tiling the file; deb requires a leading debian-binary member holding 2.0 and control/data tarballs; rlib requires a lib.rmeta member carrying the rustc metadata magic and version; vib requires a leading descriptor.xml member whose safe XML parse has a vib root naming its type and name; other member contents not interpreted"
MAGIC = b"!<arch>\n"
RUST_METADATA = b"rust\0\0\0"  # rustc METADATA_HEADER, then one version byte
HEADER = 60
MEMBERS = 65536
...
def validate(data: bytes, hints: frozenset[str]) -> Observation | None:
    ...
    if names and names[0].rstrip(b"/") == b"descriptor.xml":
        _, start, size = found[0]
        try:
            root = xml.parse(data[start : start + size])
        except (xml.Malformed, xml.Unsafe, xml.Unsupported) as error:
            return Observation("fail", f"descriptor.xml does not parse: {error}", "vib")
        name = root.findtext("name")
        if root.tag != "vib" or not root.findtext("type") or not name:
            return Observation("fail", "descriptor.xml is not a VIB descriptor", "vib")
        return Observation(
            "pass", f"{len(found)} members; VIB descriptor for {name[:60]}", "vib", tuple(tags)
        )
    for name, start, size in found:
        if name.rstrip(b"/") != b"lib.rmeta":
            continue
        # rustc wraps the metadata in an object file (ELF, COFF, Mach-O or wasm), so the
        # header sits inside a section rather than at the member start.
        at = data.find(RUST_METADATA, start, start + size - 1)
        if at < 0:
            return Observation("fail", "lib.rmeta member lacks the rustc metadata header", "rlib")
        return Observation(
            "pass",
            f"{len(found)} members; lib.rmeta carries rustc metadata version {data[at + 7]}",
            "rlib",
            tuple(tags),
        )
    return Observation(

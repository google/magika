    ...
    for tool, label, _ in TOOLS:
        ...
        for basis in ["validated_auto", "reviewed_source_mapping"]:
            indices = [
                ...
                if annotation[s["sha256"]]["evaluation_basis"] == basis
            ]
            by_basis[basis] = c.quality_metrics(
                [samples[i] for i in indices], [observed[tool][i] for i in indices]
            )
        wrong[tool] = [
            dict(
                sha256=s["sha256"],
                ...
                evaluation_basis=annotation[s["sha256"]]["evaluation_basis"],
        ...
        confusions[tool] = [
            {"truth": truth, "prediction": prediction, "files": n}
            for (truth, prediction), n in Counter(
                (r["truth"], r["prediction"]) for r in wrong[tool]
            ).most_common()
        ]
        times = {}
        for count in COUNTS:
...
def render(path, output):
    ...
    lines += [
        "",
        "The scoring subset is selected solely by the dataset lane’s existing `evaluation_eligible` flags: "
    ...
    lines[2:2] = report_identity_lines(d)

            ...
            fixtures.append(
                {
                    **common,
                    ...
                    "path": url,
                    "url": url,
                    "source_group": "site:" + urlsplit(url).netloc,
                }
            )
...
def https_url(url):
    """A plain HTTPS file URL: no credentials, port, query or fragment to smuggle state."""
    parsed = urlsplit(url)
    if (
        parsed.scheme != "https"
        ...
        raise ValueError("HTTPS origins require a plain HTTPS file URL")
    return url


class HTTPSReader:
    """Downloads an HTTPS origin without following redirects, bounded by its known size."""

    def read_into(self, fixture, output):
        remaining, digest = fixture["size"], hashlib.sha256()
        request = Request(https_url(fixture["url"]), headers={"Accept-Encoding": "identity"})
        with build_opener(NoRedirect()).open(request, timeout=30) as response:
            while remaining:
                chunk = response.read(min(1024 * 1024, remaining))
                if not chunk:
                    raise ValueError("HTTPS download truncated")
                remaining -= len(chunk)
                digest.update(chunk)
                output.write(chunk)
            if response.read(1):
                raise ValueError("HTTPS download exceeds manifest size")
        if digest.hexdigest() != fixture["sha256"]:
            raise ValueError("HTTPS download SHA-256 mismatch; the origin changed")

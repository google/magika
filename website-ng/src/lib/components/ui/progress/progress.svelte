<script lang="ts">
	...
	const percentage = $derived.by(() => Math.round(((value ?? 0) / (max ?? 1)) * 100));
</script>

<ProgressPrimitive.Root
	bind:ref
	data-slot="progress"
	class={cn("bg-primary/20 relative w-full overflow-hidden rounded-full", height, className)}
	{value}
	{max}
	{...restProps}
>
	...
	{#if showPercentage}
		<div
			class="absolute inset-0 flex mt-0 font-size sl-markdown-content items-center justify-center text-xs font-medium text-foreground z-10 text-2xl"
		>
			{percentage}%
		</div>
	{/if}

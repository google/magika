...
def stream(kind, specific):
    fields = guid(kind) + b"\0" * 16 + b"\0" * 8 + struct.pack("<II", len(specific), 0)
    return obj(asf.STREAM_PROPERTIES, fields + b"\0" * 6 + specific)


def media(*streams):
    props = obj(asf.FILE_PROPERTIES, b"\0" * 80)
    subs = [props, *streams]
    header = (
        guid(asf.HEADER)
        + struct.pack("<Q", 30 + sum(len(s) for s in subs))
    ...
    return header + obj(asf.DATA, b"\0" * 26)


def audio(tag):
    return stream(asf.AUDIO, struct.pack("<HHIIHH", tag, 2, 44100, 16000, 4, 16))


def video(fourcc):
    return stream(asf.VIDEO, b"\0" * 11 + struct.pack("<IiiHH", 40, 320, 240, 1, 24) + fourcc)


def test_windows_media_audio_is_named_by_codec():
    result = asf.validate(media(audio(0x0161)), frozenset())
    assert result.format_id == "wma" and not result.generic


def test_windows_media_video_is_named_by_its_video_codec():
    assert asf.validate(media(audio(0x0161), video(b"WMV3")), frozenset()).format_id == "wmv"


def test_other_codecs_leave_the_generic_container():
    result = asf.validate(media(audio(0x0055), video(b"MP43")), frozenset())
    assert result.format_id == "asf" and result.generic

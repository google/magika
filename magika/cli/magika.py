...
@click.option(
    ...
    "recursive_flag",
    is_flag=True,
    help="Recursively scan every directory.",
)
@click.option("--json", "json_output_flag", is_flag=True, help="Output in JSON format.")
@click.option(
    "--jsonl", "jsonl_output_flag", is_flag=True, help="Output in JSONL format."
)
@click.option(
    "-i",
    "--mime",
    "mime_output_flag",
    is_flag=True,
    help="Output the MIME type instead of a content type label.",
)
@click.option(
    "-p",
    "--with-probability",
...
@click.option(
    ...
    "output_highest_probability",
    is_flag=True,
    help="Output the content type with the highest probability, regardless of thresholds.",
)
@click.option(
    "-b", "--batch-size", default=32, help="How many files to process in one batch."
)
@click.option(
    "--colors/--no-colors",
    "with_colors_flag",
...
@click.option(
    "--version", "version_flag", is_flag=True, help="Print the version and exit."
)
@click.option(
    "-l",
    "--list",
...
def main(
    ...
    if compatibility_mode_flag:
        log.warning(
            'Support for compatibility mode is currently very limited, many outputs will be "Unknown"'
        )

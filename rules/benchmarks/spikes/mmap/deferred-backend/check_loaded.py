...
def check(distribution, destination):
    ...
    rows = []
    for mode, flags in [('rules', ['--rules=only']), ('cpu', ['--rules=off', '--backend=cpu']),
                        ('gpu', ['--rules=off', '--backend=gpu'])]:
        command = [str(distribution / 'magika'), *flags, '--batch-size=8', '--backend-info']
        # No backend path override: verify the relocatable packaged layout itself.
        env = {k: v for k, v in os.environ.items() if k not in ['MAGIKA_RUNTIME_DIR', 'MAGIKA_STARTUP_TRACE']}
        ...
        for line in text.splitlines():
            ...
            if line.endswith('move delayed to loaded: Metal'):
                metal = True
        assert (cpu, gpu, metal) == {'rules': (False, False, False), 'cpu': (True, False, False), 'gpu': (False, True, True)}[mode]
        rows.append({'mode': mode, 'command': command, 'stdout': result.stdout.decode(),
                     'cpu_library_loaded': cpu, 'metal_library_loaded': gpu, 'metal_framework_active': metal,
    ...
    with tempfile.TemporaryDirectory(prefix='magika-abi-') as temporary:
        ...
        (directory / 'libmagika_runtime_cpu.dylib').symlink_to(distribution / 'lib/libmagika_runtime_cpu.dylib')
        source = directory / 'bad.c'
        source.write_text('#include <stdint.h>\nstatic uint32_t header[] = {99, 8};\nconst void *magika_runtime_v1(void) { return header; }\n')
        subprocess.run(['cc', '-dynamiclib', str(source), '-o', str(directory / 'libmagika_runtime_metal.dylib')], check=True)
        results = []
        for backend in ['cpu', 'auto', 'gpu']:
            command = [str(distribution / 'magika'), '--backend=' + backend, '--rules=off', '--batch-size=8', '--backend-info']
            env = dict(os.environ, MAGIKA_RUNTIME_DIR=str(directory))
            result = subprocess.run(command, env=env, capture_output=True)
            if backend == 'gpu':
                assert result.returncode != 0 and b'incompatible inference backend ABI' in result.stderr
            else:
                assert result.returncode == 0 and b'tract-cpu' in result.stdout
            results.append({'backend': backend, 'returncode': result.returncode, 'stdout': result.stdout.decode(), 'stderr': result.stderr.decode()})
        (destination / 'abi-rejection.json').write_text(json.dumps(results, indent=2) + '\n')
